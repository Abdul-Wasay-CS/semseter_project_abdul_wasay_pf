...
public class Main {
    ...
    while (true) {
      ...
      try {
        ...
        if (idToAssign >= 0) {
          ...
          logs.add("Admin Account with ID-" + idToAssign + " created.\t" + getCurrentTimeAndDate());
          saveLogs();
        }
        else
        {
          out.println("All Admin slots are full, Please delete some admins to add new admin");
        }
        break;
      } catch (Exception e) {
        out.println(e.toString());
  ...
  public static void deleteAdmin() throws InterruptedException {
    while (true) {
      try {
        ...
        logs.add("Admin Account with ID-" + idToDelete + " deleted.\t" +  getCurrentTimeAndDate());
        saveLogs();
        return;
      } catch (Exception e) {
        out.println(e.toString());
  ...
  public static void createAccount() {
    ...
    logs.add(prompt + "\t" + getCurrentTimeAndDate());
    saveLogs();
  ...
  public static void deleteAccount(int index) {
    while (true) {
      try {
        ...
        logs.add("Account with ID-" + (index + 101) + " deleted!\t" + getCurrentTimeAndDate());
        saveLogs();
        return;
      } catch (Exception e) {
        out.println(e.toString());
  ...
  public static void depositMoney(int indexNumber) {
    while (true) {
      try {
        ...
        logs.add("A deposit of " + moneyToDeposit + "$ made by Account ID-" + creds.get(IDINDEX) + "\t" + getCurrentTimeAndDate());
        saveLogs();
        break;
      } catch (Exception e) {
        out.println(e.toString());
  ...
  public static void withdrawMoney(int indexNumber) {
    while (true) {
      ...
      logs.add("A withdrawal of " + moneyToWithdraw + "$ made by Account ID-" + creds.get(IDINDEX) + "\t" + getCurrentTimeAndDate());
      saveLogs();
      break;
    }
  }
  ...
  public static void transferMoney(int indexNumber) {
    ...
    if (receiverIndex != -1) {
      ...
      logs.add("Money Transfered by " + accountNames.get(indexNumber) + " to Mr/Mrs." + accountNames.get(receiverIndex) + ": " + moneyToTransfer + "$\t" + getCurrentTimeAndDate());
      saveLogs();
    }
  }
  ...
  public static void blockAccount(int index) {
    ...
    logs.add("Account ID-" + accountCredentials.get(index).get(IDINDEX) + " blocked!\t" + getCurrentTimeAndDate());
    saveLogs();
  }
  ...
  public static void recoverAccount(int accountID) {
    ...
    for (int i = 3; i > 0; i--) {
      ...
      if (!answer.equalsIgnoreCase(securityQuestion.get(indexToRecover)))
      {
        ...
        logs.add("Account ID-" + accountCredentials.get(indexToRecover).get(IDINDEX) + "  recovered!\t" + getCurrentTimeAndDate());
        saveLogs();
        return;
      }
    }
  ...
  public static void saveLogs() {
    try (FileOutputStream fos = new FileOutputStream("logs.txt", true); PrintWriter writer = new PrintWriter(fos)) {
